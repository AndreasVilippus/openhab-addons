...
class CalendarHandlerTest {
    private static final class MemoryStorage implements Storage<String> {
        ...
        @Override public @Nullable String put(String key, @Nullable String value) { return values.put(key, value); }
        @Override public @Nullable String get(String key) { return values.get(key); }
        @Override public @Nullable String remove(String key) { return values.remove(key); }
        @Override public boolean containsKey(String key) { return values.containsKey(key); }
        @Override public Collection<String> getKeys() { return values.keySet(); }
        @Override public Collection<@Nullable String> getValues() { return values.values(); }
    }
    private static final class Handler extends CalendarHandler {
        final Map<String, State> published = new HashMap<>();
        ThingStatus status = ThingStatus.UNKNOWN;
        Handler() { super(thing(), () -> ZoneOffset.UTC, new MemoryStorage()); }
        @Override protected void updateState(ChannelUID channel, State state) { published.put(channel.getId(), state); }
        @Override protected void updateStatus(ThingStatus status, ThingStatusDetail detail, @Nullable String description) { this.status = status; }
    }
    private static Thing thing() {
        return ThingBuilder.create(new ThingTypeUID("caldav", "calendar"), "test")
                .withConfiguration(new Configuration(Map.of("path", "https://example.org/calendar/", "calendarId", "calendar", "rangeStartOffset", -1, "rangeEndOffset", 1))).build();
    }
    private AccountConfiguration account() {
        AccountConfiguration c = new AccountConfiguration(); c.url = "https://example.org/"; c.username = "user"; c.password = "secret"; c.syncMode = "FULL"; return c;
    }
    ...
    @Test void publishesRangeClearsMissingEventsAndRefreshesLocally() throws Exception {
        Handler h = new Handler(); h.initialize();
        try {
            ...
            assertEquals("[]",h.published.get("events#json").toString());
        } finally { h.dispose(); }
    }
    @Test void communicationFailureKeepsCacheAndRecoveryReplacesIt() throws Exception {
        Handler h = new Handler(); h.initialize();
        try {
            h.synchronize((m,u,b,d) -> response(false), account(), () -> true);
            State json = h.published.get("events#json"), last = h.published.get("sync#last");
            h.synchronize((m,u,b,d) -> {throw new IOException("offline");}, account(), () -> true);
            assertEquals(ThingStatus.OFFLINE,h.status);
            assertEquals(json,h.published.get("events#json"));
            assertEquals(last,h.published.get("sync#last"));
            assertEquals("ERROR",h.published.get("sync#status").toString());
            h.synchronize((m,u,b,d) -> response(true), account(), () -> true);
            assertEquals(ThingStatus.ONLINE,h.status);
    ...
    @Test void completionAfterDisposeCannotPublish() throws Exception {
        Handler h = new Handler(); h.initialize();
        CountDownLatch entered = new CountDownLatch(1), release = new CountDownLatch(1);
        DavTransport transport = (m,u,b,d) -> { entered.countDown(); if (!release.await(10,TimeUnit.SECONDS)) {throw new IOException("timeout");} return response(false); };
        CompletableFuture<Void> work = CompletableFuture.runAsync(() -> {
            try { h.synchronize(transport,account(),() -> true); } catch (InterruptedException e) { Thread.currentThread().interrupt(); }
        });
        try {
            assertTrue(entered.await(10,TimeUnit.SECONDS));
            h.dispose();
            Map<String, State> before = Map.copyOf(h.published);
            release.countDown(); work.get(10,TimeUnit.SECONDS);
            assertEquals(before,h.published);
        } finally { release.countDown(); h.dispose(); }
    }
}
